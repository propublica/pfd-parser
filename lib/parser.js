...
function parser(files) {
    ...
    function processItem(item) {
        if (!item.str) return Promise.resolve(item);
    ...
    function repairWithGhostscript(data) {
        return new Promise((resolve, reject) => {
            const tmpIn = path.join(os.tmpdir(), 'pfd-repair-in-' + process.pid + '.pdf');
            const tmpOut = path.join(os.tmpdir(), 'pfd-repair-out-' + process.pid + '.pdf');
            fs.writeFileSync(tmpIn, Buffer.from(data));
            child_process.execFile('gs', [
                ...
                if (err || !fs.existsSync(tmpOut)) {
                    return reject(new Error('Ghostscript repair failed'));
                }
                const repaired = fs.readFileSync(tmpOut);
                fs.unlinkSync(tmpOut);
                resolve(new Uint8Array(repaired));
            });
        });
    }
    ...
    function parsePdfData(data) {
        return new Promise((resolve, reject) => {
            _(pdfjs.getDocument(data))
                ...
                .flatMap(processPage)
                .stopOnError((err) => {
                    console.error('[pfd-parser] Error processing file:', file && file.relative, err);
                    reject(err);
                })
                .done(() => {
    ...
    function process() {
        ...
        // parse PDF document, with ghostscript repair fallback for damaged files
        return parsePdfData(data).catch((err) => {
            if (err && err.message === 'Invalid PDF structure') {
                console.error('[pfd-parser] PDF appears damaged, attempting ghostscript repair:', file && file.relative);
                filing = new Filing();
                fdOnline = false;
                filing.file = file.relative
                    .replace('.pdf', '')
                    .replace('.PDF', '');
                return repairWithGhostscript(data).then(parsePdfData);
            }
            throw err;
        });
    }
